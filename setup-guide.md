...
The team's shared copy is the GitHub repo `Sonoma-Fyre/sonoma-fyre-research`. This repo is public, so anyone can clone it to read. To push, you need to be in the Sonoma-Fyre org, and we set up each teammate as an org owner, so ask any teammate to add you.

Option A, command line:
1. Install git (or the GitHub CLI, `gh`).
2. Run: `git clone https://github.com/Sonoma-Fyre/sonoma-fyre-research.git`
3. Note the folder it creates, for example `~/sonoma-fyre-research`.

Option B, GitHub Desktop:
1. Install GitHub Desktop and sign in.
2. File, Clone repository, pick `Sonoma-Fyre/sonoma-fyre-research`.
...
> Purpose: research support for Team Sonoma Fyre on reversing declining wine demand in Sonoma. This folder is a git clone of the shared repo (github.com/Sonoma-Fyre/sonoma-fyre-research).
> Sync rule (this repo does NOT auto-sync like Drive, and we never commit to main directly): start each session with `git checkout main` then `git pull`. Do your work on a feature branch (`git checkout -b type/short-topic`), commit, `git push -u origin <branch>`, then open a pull request for a teammate to review. Merge only after approval. If a pull or merge reports conflicts, stop and surface them rather than guessing.
